...
# PyYAML is optional for forensic AEP recovery. The core parser does not need
# OCIO YAML parsing unless an OCIO color-profile operation is requested.
try:
...
if _YAML_AVAILABLE:
    ...
    def _ignore_tag(loader: yaml.Loader, suffix: str, node: yaml.Node) -> Any:
        # OCIO writes tags as `!<ColorSpace>` etc.; discard the tag while
        # preserving the underlying YAML value.
    ...
    # Placeholder solely so importing py_aep never requires PyYAML.
    _OcioLoader = None


@lru_cache(maxsize=8)
def _load_config_cached(config_path: Path, _stamp: tuple[int, int]) -> dict | None:
    # OCIO YAML support is optional. Recovery and normal AEP parsing do not
    # need it, so a missing PyYAML installation simply means color-profile
    # enumeration is unavailable rather than making the entire application
    # fail at import time.
